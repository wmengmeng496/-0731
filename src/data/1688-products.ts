import type { SpiritualProduct } from "./spiritual-products";

export const imported1688Products: SpiritualProduct[] = [
  {
    "id": "1688-粉瓷-古银",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款粉瓷（古银）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "粉瓷（古银）",
    "form": "粉瓷（古银） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "桃花"
    ],
    "materials": [
      "粉瓷珠",
      "古银配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#f5d9dd",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款粉瓷（古银）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-粉瓷-古银.png"
  },
  {
    "id": "1688-灰瓷-古银",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款灰瓷（古银）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "灰瓷（古银）",
    "form": "灰瓷（古银） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安"
    ],
    "materials": [
      "灰瓷珠",
      "古银配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#8d9095",
      "#d6d0c5",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款灰瓷（古银）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-灰瓷-古银.png"
  },
  {
    "id": "1688-黑瓷-古银",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款黑瓷（古银）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "黑瓷（古银）",
    "form": "黑瓷（古银） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "护身"
    ],
    "materials": [
      "黑瓷珠",
      "古银配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#1f1f22",
      "#121212",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款黑瓷（古银）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-黑瓷-古银.png"
  },
  {
    "id": "1688-黑瓷-珠径12mm14颗",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款黑瓷（珠径12mm14颗）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "黑瓷（珠径12mm14颗）",
    "form": "黑瓷（珠径12mm14颗） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "护身"
    ],
    "materials": [
      "黑瓷珠"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#1f1f22",
      "#121212",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款黑瓷（珠径12mm14颗）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-黑瓷-珠径12mm14颗.png"
  },
  {
    "id": "1688-黑瓷-珠径14mm13颗",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款黑瓷（珠径14mm13颗）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "黑瓷（珠径14mm13颗）",
    "form": "黑瓷（珠径14mm13颗） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "护身"
    ],
    "materials": [
      "黑瓷珠"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#1f1f22",
      "#121212",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款黑瓷（珠径14mm13颗）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-黑瓷-珠径14mm13颗.png"
  },
  {
    "id": "1688-粉灰瓷-古银",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款粉灰瓷（古银）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "粉灰瓷（古银）",
    "form": "粉灰瓷（古银） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "桃花"
    ],
    "materials": [
      "灰瓷珠",
      "古银配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#f5d9dd",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款粉灰瓷（古银）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-粉灰瓷-古银.png"
  },
  {
    "id": "1688-马卡龙5色-古银",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款马卡龙5色（古银）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "马卡龙5色（古银）",
    "form": "马卡龙5色（古银） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安"
    ],
    "materials": [
      "古银配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#d7ebd0",
      "#f0d58c",
      "#8d9095"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款马卡龙5色（古银）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-马卡龙5色-古银.png"
  },
  {
    "id": "1688-乳白瓷-古银",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款乳白瓷（古银）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "乳白瓷（古银）",
    "form": "乳白瓷（古银） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安"
    ],
    "materials": [
      "乳白瓷珠",
      "古银配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#f6eee0",
      "#e7dfcf",
      "#d8b48a"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款乳白瓷（古银）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-乳白瓷-古银.png"
  },
  {
    "id": "1688-青花瓷-10mm珠径",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款青花瓷（10mm珠径）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "青花瓷（10mm珠径）",
    "form": "青花瓷（10mm珠径） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "智慧"
    ],
    "materials": [
      "青花瓷珠"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#f6eee0",
      "#1d5c9f",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款青花瓷（10mm珠径）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-青花瓷-10mm珠径.png"
  },
  {
    "id": "1688-白蓝粉-金色",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款白蓝粉（金色）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "白蓝粉（金色）",
    "form": "白蓝粉（金色） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "智慧",
      "桃花",
      "求财"
    ],
    "materials": [
      "金色配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#f5d9dd",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款白蓝粉（金色）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-白蓝粉-金色.png"
  },
  {
    "id": "1688-灰瓷-金色",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款灰瓷（金色）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "灰瓷（金色）",
    "form": "灰瓷（金色） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "求财"
    ],
    "materials": [
      "灰瓷珠",
      "金色配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#8d9095",
      "#d6d0c5",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款灰瓷（金色）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-灰瓷-金色.png"
  },
  {
    "id": "1688-墨绿瓷-金色",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款墨绿瓷（金色）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "墨绿瓷（金色）",
    "form": "墨绿瓷（金色） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "求财"
    ],
    "materials": [
      "墨绿瓷珠",
      "金色配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#2f4f3c",
      "#0f2c1e",
      "#d8b48a"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款墨绿瓷（金色）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-墨绿瓷-金色.png"
  },
  {
    "id": "1688-粉白瓷-金色",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款粉白瓷（金色）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "粉白瓷（金色）",
    "form": "粉白瓷（金色） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "桃花",
      "求财"
    ],
    "materials": [
      "金色配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#f5d9dd",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款粉白瓷（金色）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-粉白瓷-金色.png"
  },
  {
    "id": "1688-黄色瓷-金色",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款黄色瓷（金色）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "黄色瓷（金色）",
    "form": "黄色瓷（金色） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "求财"
    ],
    "materials": [
      "黄色瓷珠",
      "金色配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a84c",
      "#f0d58c",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款黄色瓷（金色）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-黄色瓷-金色.png"
  },
  {
    "id": "1688-白色珠-金色",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款白色珠（金色）手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "白色珠（金色）",
    "form": "白色珠（金色） + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "求财"
    ],
    "materials": [
      "乳白瓷珠",
      "金色配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#f6eee0",
      "#e7dfcf",
      "#d8b48a"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款白色珠（金色）手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-白色珠-金色.png"
  },
  {
    "id": "1688-粉色转经筒",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款粉色转经筒手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "粉色转经筒",
    "form": "粉色转经筒 + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安",
      "桃花"
    ],
    "materials": [
      "转经筒配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#f5d9dd",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款粉色转经筒手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-粉色转经筒.png"
  },
  {
    "id": "1688-灰色转经筒",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款灰色转经筒手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "灰色转经筒",
    "form": "灰色转经筒 + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安"
    ],
    "materials": [
      "转经筒配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#8d9095",
      "#d6d0c5",
      "#c8c0b4"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款灰色转经筒手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-灰色转经筒.png"
  },
  {
    "id": "1688-珠光多色佛头款",
    "tradition": "eastern",
    "category": "temple",
    "name": "寺庙同款珠光多色佛头款手串",
    "source": "义乌天义电子商务",
    "sourceOfferId": 897348993194,
    "sourceSkuName": "珠光多色佛头款",
    "form": "珠光多色佛头款 + 宫廷风配件",
    "supplyPrice": 5,
    "intention": [
      "颜值",
      "转运",
      "平安"
    ],
    "materials": [
      "佛头配件"
    ],
    "summary": "寺庙同款宫廷风瓷珠手串，适合做低门槛引流款、颜值款和来图定制款。",
    "care": "瓷珠避免摔碰和硬物摩擦；金属配件用软布擦拭，日常保持干燥。",
    "palette": [
      "#d8a9b7",
      "#d7ebd0",
      "#f0d58c",
      "#8d9095"
    ],
    "sellingPoints": [
      "供应价低，适合做引流款",
      "颜色 SKU 丰富，方便按命盘/星座推荐",
      "宫廷风视觉明确，适合手机端商品图展示"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 69
      },
      {
        "name": "净化包装款",
        "price": 99
      },
      {
        "name": "开光礼盒款",
        "price": 168
      }
    ],
    "consultPrompt": "我想咨询寺庙同款珠光多色佛头款手串，想看实拍、手围和是否可做开光礼盒。",
    "price": 69,
    "originalPrice": 99,
    "image": "/images/1688-products/1688-珠光多色佛头款.png"
  },
  {
    "id": "1688-red-agate-four-leaf-clover-bracelet",
    "tradition": "eastern",
    "category": "crystal",
    "name": "天然红玛瑙幸运四叶草手链",
    "source": "海丰县咏彩珠宝有限公司",
    "sourceOfferId": 706460704772,
    "sourceSkuName": "红玛瑙8mm",
    "form": "红玛瑙8mm + 四叶草配件 + 可调节女款手链",
    "supplyPrice": 11.5,
    "intention": [
      "颜值",
      "好运",
      "桃花",
      "气色"
    ],
    "materials": [
      "红玛瑙",
      "四叶草配件",
      "金色调节链"
    ],
    "summary": "红玛瑙小众轻奢手链，视觉记忆点集中在四叶草与红色圆珠，适合作为好运、气色和日常搭配款。",
    "care": "玛瑙避免硬物磕碰和化学品接触；金属配件日常保持干燥，收纳时单独放置减少刮花。",
    "palette": [
      "#9d2f2f",
      "#c83a32",
      "#d8a84c",
      "#f4e6d7"
    ],
    "sellingPoints": [
      "天然红玛瑙色彩醒目，适合手机端商品首图",
      "四叶草造型有好运寓意，方便做礼物款和节日款",
      "可调节手链门槛低，适合年轻女性日常佩戴"
    ],
    "skuOptions": [
      {
        "name": "基础现货款",
        "price": 49
      },
      {
        "name": "好运包装款",
        "price": 69
      },
      {
        "name": "净化礼盒款",
        "price": 99
      }
    ],
    "consultPrompt": "我想咨询天然红玛瑙幸运四叶草手链，想看实拍、手围调节范围和礼盒包装。",
    "price": 49,
    "originalPrice": 69,
    "image": "/images/1688-products/red-agate-four-leaf-clover-preview.svg"
  }
];
